...
struct SCN_DirectionalLight::Spec : public AnonObjectSpec {
...
void SCN_DirectionalLight::s_initRtti(axType* type) {
	static Spec spec(type);
}

const SCN_DirectionalLight::Spec* SCN_DirectionalLight::Spec::s_instance = nullptr;

void SCN_DirectionalLight::onInitProps() {
	Base::onInitProps();
}

bool SCN_DirectionalLight::onReadJsonProp(axJsonReader& rd_) {
	bool _ret = Base::onReadJsonProp(rd_);
	return _ret;
}

void SCN_DirectionalLight::onWriteJsonProp(axJsonWriter& wr_) const {
