...
AX_REFLECTION_IO_FUNC_IMPL(	template<class T>, axColorRGBA_<T>)

template<class T>
void axColorRGBA_<T>::toHexString(axIString& s) const {
	auto* hex = axCStr::kHexChars();
	s.clear();
	s.reserve(10);
	auto tmp = axColorRGBAb::s_make(*this);
	...
	s.append(hex[(tmp.a.value >> 4) & 0xf], hex[tmp.a.value & 0xf]);
}

#define	AX_MACRO_OP(T, VALUE, SUFFIX)	\
	template class axColorRGBA_<T>; \
//-------------
